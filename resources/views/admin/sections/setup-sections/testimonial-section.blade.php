                        ...
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                ...
                                <td><a href="{{ $item->language->$system_default_lang->link ?? '' }}"
                                        class="text--danger">{{ $item->language->$system_default_lang->item_name ?? '' }}</a>
                                </td>
                                <td><a href="{{ $item->language->$system_default_lang->link ?? '' }}"
                                        class="text--danger">{{ $item->language->$system_default_lang->item_testimonial_title ?? '' }}</a>
                                </td>
                                <td><a href="{{ $item->language->$system_default_lang->link ?? '' }}"
                                        class="text--danger">{{ $item->language->$system_default_lang->item_testimonial_rating ?? '' }}</a>
                                </td>
                                <td><a href="{{ $item->language->$system_default_lang->link ?? '' }}"
                                        class="text--danger">{{ date('d-m-Y', strtotime($item->created_at)) ?? '' }}</a>
                                </td>
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                ...
                                <div class="tab-pane @if (get_default_language_code() == language_const()::NOT_REMOVABLE) fade show active @endif"
                                    ...
                                    <div class="form-group">
                                        @include('admin.components.form.input', [
                                            'label' => __('Name') . '*',
                                            'name' => $default_lang_code . '_item_name_edit',
                                            'value' => old(
                                                $default_lang_code . '_item_name_edit',
                                                $data->value->language->$default_lang_code->item_name ?? ''),
                                        ])
                                    </div>
                                    ...
                                    <div class="form-group">
                                        @include('admin.components.form.input', [
                                            'label' => __('Title') . '*',
                                            'name' => $default_lang_code . '_item_testimonial_title_edit',
                                            'value' => old(
                                                $default_lang_code . '_item_testimonial_title_edit',
                                                $data->value->language->$default_lang_code->item_testimonial_title ?? ''),
                                        ])
                                    </div>
                                    <div class="form-group">
                                        @include('admin.components.form.input', [
                                            'label' => __('Rating') . '*',
                                            'name' => $default_lang_code . '_item_testimonial_rating_edit',
                                            'value' => old(
                                                $default_lang_code . '_item_testimonial_rating_edit',
                                                $data->value->language->$default_lang_code->item_testimonial_rating ?? ''),
                                        ])
                                    </div>
                                    ...
                                    <div class="form-group">
                                        @include('admin.components.form.textarea', [
                                            ...
                                            'name' => $default_lang_code . '_item_testimonial_description_edit',
                                            'value' => old(
                                                $default_lang_code . '_item_testimonial_description_edit',
                                                $data->value->language->$default_lang_code->item_testimonial_description ?? ''),
                                        ])
                                    </div>
                                ...
                                @foreach ($languages as $item)
                                    ...
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif"
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.input', [
                                                'label' => __('Title')."*",
                                                'name' => $lang_code . '_item_testimonial_title_edit',
                                                'value' => old(
                                                    $lang_code . '_item_testimonial_title_edit',
                                                    $data->value->language->$lang_code->item_testimonial_title ??
                                                        ''),
                                            ])
                                        </div>
                                        <div class="form-group">
                                            @include('admin.components.form.input', [
                                                'label' => __('Rating') . '*',
                                                'name' => $lang_code . '_item_testimonial_rating_edit',
                                                'value' => old(
                                                    $lang_code . '_item_testimonial_rating_edit',
                                                    $data->value->language->$lang_code->item_testimonial_rating ??
                                                        ''),
                                            ])
                                        </div>
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.textarea', [
                                                'label' => __('Description')."*",
                                                'name' => $lang_code . '_item_testimonial_description_edit',
                                                'value' => old(
                                                    $lang_code . '_item_testimonial_description_edit',
                                                    $data->value->language->$lang_code->item_testimonial_description ?? ''),
                                            ])
                                        </div>
    ...
    <script>
        ...
        $(".edit-modal-button").click(function() {
            ...
            editModal.find("input[name=" + default_language + "_item_name_edit]").val(oldData.language[
                    default_language]
                .item_name);
            editModal.find("input[name=" + default_language + "_item_designation_edit]").val(oldData.language[
                default_language].item_designation);
            editModal.find("input[name=" + default_language + "_item_testimonial_title_edit]").val(oldData.language[
                    default_language]
                .item_testimonial_title);
            editModal.find("input[name=" + default_language + "_item_testimonial_rating_edit]").val(oldData
                .language[
                    default_language]
                .item_testimonial_rating);
            editModal.find("textarea[name=" + default_language + "_item_testimonial_description_edit]").val(oldData
                .language[
                    default_language]
                .item_testimonial_description);

            $.each(languages, function(index, item) {
                editModal.find("input[name=" + item.code + "_item_name_edit]").val(oldData.language[item
                    .code]?.item_name);
                editModal.find("input[name=" + item.code + "_item_designation_edit]").val(oldData.language[
                    item.code]?.item_designation);
                editModal.find("input[name=" + item.code + "_item_testimonial_title_edit]").val(oldData
                    .language[
                        item
                        .code]?.item_testimonial_title);
                editModal.find("input[name=" + item.code + "_item_testimonial_rating_edit]").val(oldData
                    .language[
                        item
                        .code]?.item_testimonial_rating);
                editModal.find("textarea[name=" + item.code + "_item_testimonial_description_edit]").val(
                    oldData
                    .language[
                        item
                        .code]?.item_testimonial_description);
            });
            console.log(oldData)
