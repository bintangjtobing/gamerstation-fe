...
@section('content')
    <form action="{{ setRoute('admin.payment.gateway.update', ['add-money', 'automatic', $gateway->alias]) }}" method="POST"
            ...
            <div class="card-body">
                <div class="row mb-10-none">
                    ...
                    <div class="col-xl-7 col-lg-7">
                        @include('admin.components.payment-gateway.automatic.credentials', [
                            'gateway' => $gateway,
                        ])
                        ...
                        <div class="col-xl-3 col-lg-3 col-md-6 col-sm-6 form-group">
                            @include('admin.components.form.switcher', [
                                ...
                                'name' => 'mode',
                                'options' => [
                                    'Production' => payment_gateway_const()::ENV_PRODUCTION,
                                    'Sandbox' => payment_gateway_const()::ENV_SANDBOX,
                                ],
                            ])
                        </div>

                        @if ($gateway->isRazorpay($gateway))
                            {{ __('Callback URL for Addmoney') }} <br>
                            {{ setRoute('user.add.money.payment.callback', [$gateway->alias, 'token' => payment_gateway_const()::CALLBACK_HANDLE_INTERNAL]) }}
                            <br><br>
                            {{ __('Callback URL for Top Up') }} <br>
                            {{ setRoute('top.up.payment.callback', [$gateway->alias, 'token' => payment_gateway_const()::CALLBACK_HANDLE_INTERNAL]) }}
                        @endif
                    </div>
                    <div class="col-xl-2 col-lg-2 form-group">
                        @include(
                            'admin.components.payment-gateway.automatic.supported-currencies',
                            compact('gateway'))
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function() {
            $(".payment-gateway-currency").change(function(event) {
                ...
                if ($(this).is(":checked")) {
                    ...
                    var paymentGatewayCurrenciesWrapper = getHtmlMarkup()
                        .payment_gateway_currencies_wrapper;
                    ...
                    if (credentialsElement.siblings(".payment-gateway-currencies-wrapper").length > 0) {
                        ...
                        $(".payment-gateway-currencies-wrapper .gateway-currency").removeClass(
                            "last-added");

                        var firstGatewayCurrencyItem = $(
                            ".payment-gateway-currencies-wrapper .gateway-currency").first();
                    ...
                    if (target == undefined) {
                        ...
                        var alertMessage = "Are you sure to remove <strong>" + paymentGatewayCurrencyBlock
                            .find(".currency-title").html() + "</strong> ?";
                        ...
                        $(".gateway-remove-btn").click(function() {
                            ...
                            $.post("{{ setRoute('admin.payment.gateway.currency.remove') }}", {
                                _method: "DELETE",
                                _token: CSRF,
                                data_target: target
                            }, function(response) {
