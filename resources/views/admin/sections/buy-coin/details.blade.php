                ...
                <div class="row align-items-center mb-10-none">
                    <div class="col-xl-4 col-lg-4 form-group">
                        <ul class="user-profile-list-two">
                            ...
                            <li class="two">{{ __('User Name') }}: <span>{{ @$data->user->username }}</span></li>
                            <li class="three">{{ __('Email') }}:
                                <span>{{ isset($data->user) ? $data->user->email : 'N/A' }}</span>
                            </li>
                            <li class="one">{{ __('Login Vender') }}: <span>{{ @$data->login_vender }}
                                    {{ @$data->currency->currency_code }}</span></li>
                            <li class="five">{{ __('Amount') }}: <span>{{ number_format(@$data->price, 2) }}
                    ...
                    <div class="col-xl-4 col-lg-4 form-group">
                        <ul class="user-profile-list two">
                            ...
                            <li class="four">{{ __('Recharge Coin') }}: <span>{{ @$data->coin }}</span></li>
                            <li class="two">{{ __('Charge') }}:
                                <span>{{ number_format(@$data->total_amount - $data->price, 2) }}
                                    {{ get_default_currency_code() }}
                                    {{ @$data->currency->currency_code }}</span>
                            </li>
                            <li class="three">{{ __('After Charge') }}: <span>
                                    {{ number_format(@$data->price + $data->price, 2) }}
                                    {{ get_default_currency_code() }}</span></li>
                            <li class="four">{{ __('Payable') }}: <span>{{ number_format(@$data->total_amount, 2) }}
                                    {{ get_default_currency_code() }}</span></li>
