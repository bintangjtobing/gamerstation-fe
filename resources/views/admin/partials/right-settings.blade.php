        ...
        <div class="language-area">
            ...
            <div class="radio-wrapper">
                <div class="radio-item">
                    <input type="radio" id="test-default" value="en" name="lang_switch"
                        @if (app()->currentLocale() == language_const()::NOT_REMOVABLE) checked @endif>
                    <label for="test-default">English</label>
                </div>
                @foreach ($__languages->where('code', '!=', language_const()::NOT_REMOVABLE) as $key => $item)
                    <div class="radio-item">
                        <input type="radio" id="test{{ $key }}" value="{{ $item->code }}"
                            name="lang_switch" @if (app()->currentLocale() == $item->code) checked @endif>
...
@push('script')
    <script>
        $("input[name=lang_switch]").change(function() {
            var submitForm =
                `<form action="{{ setRoute('admin.languages.switch') }}" id="local_submit" method="POST"> @csrf <input type="hidden" name="target" value="${$(this).val()}" ></form>`;
