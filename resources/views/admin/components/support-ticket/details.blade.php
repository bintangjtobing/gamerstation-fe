        ...
        <div class="support-profile-header">
            <div class="custom-check-group two mb-0">
                @if (Str::is('admin.*', Route::currentRouteName()))
                    <input type="checkbox" class="solve-checkbox" id="action"
                        @if ($support_ticket->status == support_ticket_const()::SOLVED) @checked(true) @endif>
                    <label for="action">{{ __('Mark as Solved') }}</label>
                @else
                    <span
                        class="{{ $support_ticket->stringStatus->class }}">{{ __($support_ticket->stringStatus->value) }}</span>
                @endif
            </div>
            ...
            <script>
                ...
                $(".solve-checkbox").change(function() {
                    if ($(this).is(":checked")) {
                        ...
                        var message =
                            `Are you sure to mark as solved (Token: <strong>${target}</strong>)? Because it's not reversable.`;
